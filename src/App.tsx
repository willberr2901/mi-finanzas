...
function NavBar() {
  ...
  return (
    <>
      <button onClick={lockNow} className="fixed top-4 right-4 z-40 w-10 h-10 rounded-full bg-slate-800/90 backdrop-blur border border-white/10 flex items-center justify-center hover:bg-slate-700 transition-colors" title="Bloquear app">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 pb-safe">
        <div className="flex justify-around items-center px-2 py-3 bg-slate-900/95 backdrop-blur-md border-t border-white/10">
          {menuItems.map((item) => {
            ...
            return (
              <button key={item.path} onClick={() => window.location.hash = item.path} className={`flex flex-col items-center py-1 px-2 transition-all duration-300 ${isActive ? 'text-emerald-400 scale-110' : 'text-slate-500 hover:text-slate-300'}`}>
      ...
      <button onClick={() => window.location.hash = '/rentabilidad'} className="fixed bottom-24 left-1/2 -translate-x-1/2 w-14 h-14 rounded-full bg-gradient-to-tr from-emerald-400 to-cyan-500 flex items-center justify-center text-black shadow-[0_0_25px_rgba(16,185,129,0.5)] z-40 hover:scale-110 transition-transform">
            ...
            <div className="bg-slate-900 w-full max-w-sm rounded-2xl p-6 border border-emerald-500/30 shadow-2xl">
              <div className="text-center">
                <div className="mx-auto w-16 h-16 bg-emerald-500/20 rounded-full flex items-center justify-center mb-4">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                </div>
                ...
                <button onClick={() => { setShowUpdatePopup(false); window.location.reload(); }} className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-black font-bold py-3 px-6 rounded-xl shadow-lg">Actualizar Ahora</button>
              </div>
            </div>
          </div>
        )}
